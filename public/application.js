...
var store = {
  // Ui.js - nextMove- store.last round captures the reponse with all of the board info including the most recent move
  //  Ui.js - newGameStart - stores the starting information when a new game is created
  //  events.js - updateGame - stores the value, or index, of the most recent move
};
...
var addThingsToBoard = function addThingsToBoard(response) {
  for (var i = 8; i >= 0; i--) {
...
var toggleXandO = function toggleXandO() {
  var value = 'x';
...
module.exports = {
  addThingsToBoard: addThingsToBoard,
  toggleXandO: toggleXandO
};
...
var winTie = __webpack_require__(12);
...
var startGame = function startGame() {
  ...
  api.newGame().then(ui.newGameStart).catch(ui.fail);
};
...
var updateGame = function updateGame() {
  ...
  if (store.game.cells[this.id] === '' && store.game.over === false) {
    ...
    api.updateGame(data).then(ui.nextMove).catch(ui.fail);
  } else {
    var textBox = $('#winbox');
...
var getPastGames = function getPastGames() {
  event.preventDefault();

  api.getPastGames().then(ui.pastGames).catch(ui.fail);
};
...
var wipeGames = function wipeGames() {
  event.preventDefault();
  $('#games-holder').html('');
};
...
"use strict";
/* WEBPACK VAR INJECTION */(function($) {// for trivia, create an array that has numbers added to it after each click, based off of the length display different trivia, len = 1 display id 1 etc. at end of game, reset the array.
...
var newGameStart = function newGameStart(response) {
...
var nextMove = function nextMove(response) {
  store.game = response.game;
  gamePlay.addThingsToBoard(response);
  $('#winbox').html('');
  $('#fail').hide();
...
var isGameOver = function isGameOver(id) {
  var lettervalue = gamePlay.toggleXandO();
  store.game.cells.splice(id, 1, lettervalue);
  var value = false;
  var tie = false;
  var checkForBlank = function checkForBlank(first, second, third) {
